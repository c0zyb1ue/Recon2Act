...
class SO101Leader(Device):
    ...
    def __init__(
        ...
        self._read_num_retry = int(os.environ.get("LEISAAC_SO101_READ_RETRIES", "5"))
        self._read_retry_delay = float(os.environ.get("LEISAAC_SO101_READ_RETRY_DELAY", "0.01"))
    ...
    def get_device_state(self):
        last_error = None
        for attempt in range(self._read_num_retry + 1):
            try:
                state = self._bus.sync_read("Present_Position", num_retry=1)
                self._last_device_state = state
                return state
            except ConnectionError as exc:
                last_error = exc
                if attempt < self._read_num_retry:
                    time.sleep(self._read_retry_delay)

        now = time.monotonic()
        if self._last_device_state is not None:
            if now - self._last_read_warning_time > 1.0:
                print(
                    "[WARN] SO101-Leader Present_Position read failed; "
                    f"using last valid state. Error: {last_error}"
                )
                self._last_read_warning_time = now
            return self._last_device_state

        raise ConnectionError(
            "SO101-Leader failed to read Present_Position and has no previous valid state. "
            "Check power, USB cable, motor IDs 1-6, and port setting. "
