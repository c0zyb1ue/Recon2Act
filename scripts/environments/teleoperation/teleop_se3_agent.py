...
_LOCAL_LEISAAC_SOURCE = Path(__file__).resolve().parents[3] / "source" / "leisaac"
if _LOCAL_LEISAAC_SOURCE.exists():
    sys.path.insert(0, str(_LOCAL_LEISAAC_SOURCE))
...
def main():  # noqa: C901
    ...
    printed_robot_root_after_first_step = False
        ...
        while simulation_app.is_running() and not interrupted:
            ...
            with torch.inference_mode():
                ...
                # apply actions
                    ...
                    env.step(actions)
                    if not printed_robot_root_after_first_step:
                        try:
                            ...
                            if root_pos is not None and root_quat is not None:
                                print(
                                    "[DEBUG] robot root pose after first sim step: "
                                    f"pos={root_pos.detach().cpu().tolist()} "
                                    f"quat={root_quat.detach().cpu().tolist()}"
                                )
                            else:
                                ...
                                if root_state is not None:
                                    print(
                                        "[DEBUG] robot root pose after first sim step: "
                                        f"{root_state[:, :7].detach().cpu().tolist()}"
                                    )
                        except Exception as exc:
                            print(f"[DEBUG] robot root pose after first sim step unavailable: {exc}")
                        printed_robot_root_after_first_step = True
                if rate_limiter:
